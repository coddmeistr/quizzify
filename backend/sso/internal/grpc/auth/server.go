...
func (s *serverAPI) Register(ctx context.Context, req *ssov1.RegisterRequest) (*ssov1.RegisterResponse, error) {
	...
	if err != nil {
		if errors.Is(err, auth.ErrUserExists) { // Not sure we should take errors directly from Auth
...
func (s *serverAPI) Login(ctx context.Context, req *ssov1.LoginRequest) (*ssov1.LoginResponse, error) {
	...
	if err != nil {
		if errors.Is(err, auth.ErrInvalidCredentials) { // Not sure we should take errors directly from Auth
			return nil, status.Error(codes.InvalidArgument, "Invalid credentials or user not exists")
		}
		if errors.Is(err, auth.ErrAppNotFound) { // Not sure we should take errors directly from Auth
...
func (s *serverAPI) IsAdmin(ctx context.Context, req *ssov1.IsAdminRequest) (*ssov1.IsAdminResponse, error) {
	...
	if err != nil {
		if errors.Is(err, auth.ErrUserNotFound) { // Not sure we should take errors directly from Auth
